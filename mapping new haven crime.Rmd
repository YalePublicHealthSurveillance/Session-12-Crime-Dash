...
```{r}
sub1 <- n2[n2$NIBRS_Offe %in% c('DISORDERLY CONDUCT') , ]
p1 <- crimeplot.fun(ds=sub1,select.crimes="DISORDERLY CONDUCT",
                    opacity1=0.1)
```
...
There is a great gallery of examples with code you can modify here: https://shiny.rstudio.com/gallery/ We are going to use an add on to Shiny called ShinyDashboard, which makes things look even nicer.
...
It is saving an output called "plot_time" that can be accessed by the UI by saving the plot to output$plot_time 
To make the output appear, add the the plotOutput("plot_time") statement in the mainPanel
